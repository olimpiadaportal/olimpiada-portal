// The password strength rule, in ONE place — mobile-app copy.
//
...
// accounts.
//
// THIS FILE IS A TWIN of `web-app/src/lib/auth/passwordPolicy.ts`. mobile-app cannot import web-app
// (tsconfig maps only `@/* -> ./src/*`), and this copy is CLIENT-SIDE
// FEEDBACK ONLY — the authoritative check runs in the BFF. It exists so the
// user is told the rule before submitting, not so the rule is enforced here.
