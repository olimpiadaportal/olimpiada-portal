...
export default async function ChildDashboard() {
  ...
  const { data: student } = await supabase
    .from("students")
    .select("first_name, last_name, access_status, child_unique_id")
  ...
  // Self-service profile: display name, 8-digit login ID, avatar (degrades to
  // initials when none / on any read failure — never crashes the dashboard).
  ...
  // Translated strings handed to the client profile components (no client i18n).
    ...
    <>
        ...
        <div className="arena-ticker-track">
          {Array.from({ length: 2 }).map((_, i) => (
            <span key={i}>
              ...
              &nbsp;·&nbsp; <b>{t("arena.tickerToday")}</b> · OLIMP·ARENA &nbsp;·&nbsp;{" "}
            </span>
          ))}
        </div>
      </div>
      ...
      <section className="arena-cols">
        ...
        <div>
          ...
          {/* ---- Profile / settings: own profile + wallpaper ---- */}
          <h3 className="arena-section-h" style={{ marginTop: 26 }}>
            {t("profile.title")}
          </h3>
          <div className="arena-panel">
          ...
          <div className="arena-panel">
            ...
            <WallpaperPicker wallpapers={(wallpapers ?? []) as any} currentId={currentId} />
