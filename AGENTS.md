...
- **Gender declaration — SETTLED 2026-09-15. Do not re-open it as a blocker.** A `GENDER DECLARATION BLOCKER` bullet stood here from 2026-09-09: 1.16.0 is the first build collecting the optional child `gender` field, and neither Play *Data safety* nor App Store *App Privacy* declared it. The owner updated BOTH consoles before submitting, and Apple approved 1.16.0 on 2026-09-15 with the field declared. `mobile-app/__tests__/ota-data-safety-freeze.test.ts` was deleted in the same change, which is exactly how that file documented its own retirement. What SURVIVES the retirement, because it is the general rule the specific blocker was an instance of: **a store declaration is part of the submission, so any change to what the app collects about a child updates BOTH forms BEFORE the build that collects it is submitted — never alongside, never after.** Neither console needs a new build for that; App Privacy publishes from App Store Connect on its own and the Play form is submitted separately from a release. The inventory to fill them from is `mobile-app/markdowns/STORE_LAUNCH_PACK.md` §2, the console steps are `docs/STORE_LISTING_COPY.md` §8.1, and `node mobile-app/scripts/submission-preflight.mjs` names it among the checks no script can verify — a SKIP there is still not a pass.
