...
export const messages: Record<Locale, Record<string, string>> = {
  az: {
    ...
    // ONE ROUTE IN, AND NO APPROVAL BEHIND IT (migration 181). The invite code
    // came back on 2026-09-17 alongside the credential route that had replaced
    // it in 177, and the owner withdrew the credential route the same day: a
    // code is a one-time, expiring secret the CREATING parent hands over
    // deliberately, while a child's password is a standing one the child can
    // pass to anyone. Redeeming CREATES the link, so nothing here may promise a
    // review, a request or a wait.
    //
    // DELETED RATHER THAN LEFT UNUSED, and both sets for the same reason — a
    // ready-made string is how a future round rebuilds a flow that was removed
    // on purpose: link.pending / link.approve / link.reject (the approval step
    // that completed zero links) and link.childPassword / link.credentialsHint /
    // link.err.credentialsInvalid / link.err.alreadyOwner (the credential
    // route). link.err.alreadyOwner went with them because the RPC dropped in
    // 181 was the only thing that could ever emit it.
    "link.subtitle":
      "Uşaq artıq başqa valideyn tərəfindən yaradılıbsa, profili yaradan valideyndən dəvət kodu istəyin. Kodu daxil etdiyiniz anda giriş açılır — təsdiq gözləmək lazım deyil.",
    "link.childId": "Uşağın ID-si",
    "link.enterCode.title": "Dəvət kodunu daxil edin",
    "link.enterCode.body":
      "Profili yaradan valideyn sizin üçün birdəfəlik dəvət kodu yaradır. Kodu və uşağın 8 rəqəmli ID-sini ondan alın.",
    "link.code": "Dəvət kodu",
    "link.codeHint": "Kod birdəfəlikdir və 72 saatdan sonra etibarsız olur.",
  ...
  en: {
    ...
    "link.subtitle":
      "If another parent already created the child, ask them for an invitation code. Access opens the moment you enter it — there is nothing to approve and nothing to wait for.",
    "link.childId": "Child's 8-digit ID",
    "link.enterCode.title": "Enter the invitation code",
    "link.enterCode.body":
      "The parent who created the profile generates a single-use code for you. Ask them for the code and the child's 8-digit ID.",
    "link.code": "Invitation code",
    "link.codeHint": "The code is single-use and stops working after 72 hours.",
  ...
  ru: {
    ...
    "link.subtitle":
      "Если ребёнка уже создал другой родитель, попросите у него код приглашения. Доступ открывается сразу после ввода кода — ничего подтверждать и ждать не нужно.",
    "link.childId": "8-значный ID ребёнка",
    "link.enterCode.title": "Введите код приглашения",
    "link.enterCode.body":
      "Родитель, создавший профиль, создаёт для вас одноразовый код. Попросите у него код и 8-значный ID ребёнка.",
    "link.code": "Код приглашения",
    "link.codeHint": "Код одноразовый и перестаёт действовать через 72 часа.",
