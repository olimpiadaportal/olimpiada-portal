...
-- linking an existing child is gone; the invite code (manage_child_link above)
-- is the only route in. Removed with it: link_child_by_verified_credentials and
-- the two throttle helpers that guarded it against becoming a password oracle
-- over an 8-digit id space.
--
-- It shipped 2026-09-16 and was withdrawn on the 17th having never been used:
-- production held 0 verification attempts and 0 grants. An invite code is a
-- one-time, expiring secret the CREATING parent issues deliberately; a child's
-- password is a standing secret the child also knows and can pass on. Of two
-- overlapping doors, the weaker one went.
--
-- child_access_adults below SURVIVES: it was never about credentials, and the
-- invite panel reads it for the "who can reach this child" list.
