...
// students) — `basePath` parameterizes every internal href so "View all",
// sorting, pagination and article links stay INSIDE the surface the reader came
// from. The public page keeps its news_public feature gate; panel surfaces are
// in-app news and render regardless of that flag (the flag governs the PUBLIC
// site section only, per the product model).
const PAGE_SIZE = 6;
...
export async function NewsBrowser({
  ...
  if (isSupabaseConfigured) {
    ...
    // RLS exposes only published news. The cover image path comes from the
    // linked media_assets row; news-media is a public bucket.
    let query = supabase
      .from("news")
      .select(
  ...
  const sortHref = (s: Sort) => `${basePath}?sort=${s}`;
  const pageHref = (p: number) => `${basePath}?sort=${sort}&page=${p}`;
    ...
    <>
        ...
        <div className="news-sort">
          {SORTS.map((s) => (
            <Link
              key={s.key}
              href={sortHref(s.key)}
              className={sort === s.key ? "news-sort-btn active" : "news-sort-btn"}
      ...
      {items.length === 0 ? (
        ...
        <div className="news-grid">
          {items.map((it, i) => (
            <Link className="news-card" href={`${basePath}/${it.slug}`} key={it.slug}>
              <div className={it.cover ? "news-card-media" : "news-card-media empty"}>
                {it.cover && (
                  // R10 (F9): the whole first page of covers loads EAGERLY (the
                  // grid is above the fold); the first two carry priority /
                  // fetchpriority=high. Combined with images.minimumCacheTTL in
                  // next.config, covers no longer pop in late on revisit.
                  <Image
                    src={it.cover}
                    alt=""
                    width={640}
              ...
              <div className="news-card-body">
                ...
                <div className="news-card-meta">
                  ...
                  <span className="views-chip">♥ {it.likes}</span>
                </div>
              </div>
            </Link>
          ))}
