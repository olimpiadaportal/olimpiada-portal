...
- For paid apps or in-app purchases you additionally need banking and tax forms (Paid Applications Agreement, W-8BEN or W-8BEN-E, bank account for payouts). ⛔ **This line used to say those agreements were not required because the app was free. IAP shipped in 1.15.0, so they ARE required — the condition it named ("if IAP is ever added") has been met.** The Free Applications Agreement alone is no longer enough. Two things follow, and they are different:
  ...
  - **A payout bank account is a separate step, and it is DONE.** Verified in App Store Connect on 2026-09-10: **International Bank of Azerbaijan OJSC, Active, bank currency EUR, royalty currency USD**, with W-8BEN and the Certificate of Foreign Status both Active. Nothing here is outstanding. *(This line previously called it the open blocker and described an AZN-only account — resolved by opening a separate EUR account at the same bank, since Azerbaijani IBANs encode the currency. Details in root `CLAUDE.md` → "Who The Seller Is".)* The reason it mattered is worth keeping: Apple holds proceeds when banking is incomplete — money accrues and simply never arrives, with no rejection and no warning.
