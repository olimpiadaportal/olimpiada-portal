...
function checkManual() {
  ...
  record(
    ...
    "The parent phone became OPTIONAL on 2026-08-31 (Apple 5.1.1(v)) and it is the only field in its Play type, so the type follows it: change Phone number from \"required\" to \"users can choose\". This is the one answer that moves toward LESS collection, which is exactly why it gets forgotten — but a form claiming a field is required when a parent can skip it is as false as the reverse. The trilingual privacy policy already describes the field as optional; only the Play Console answer remains.",
  );
  record(
    SKIP,
    "data-safety declarations cover the child gender field",
    "migration 169 added an optional child gender, collected in Add-Child and the child edit screen. Play Data safety (Personal info > Other info, Collected, purposes Analytics AND Account management) and Apple App Privacy (Other Data, linked, not tracking, purposes Analytics AND App Functionality) must BOTH list it before the build that collects it is submitted — two purposes on each form because the privacy policy also tells parents that authorised staff read the answer on the child's profile and in the account reports they export, which is not analytics. The Play OPTIONALITY answer for that type is \"Data collection is required\" — NOT \"users can choose\", which is what this line said until 2026-09-08. Play asks the question per TYPE, not per field, and Personal info > Other info now also carries the mandatory grade and school; the gender FIELD became mandatory in the product itself on 2026-09-16 (migration 178), so every field in that type is required now and the submitted console answer needs no change — and even while the field WAS optional, answering \"users can choose\" for a type that also carries two mandatory fields would have been the false statement. The inventory to fill both forms in from is mobile-app/markdowns/STORE_LAUNCH_PACK.md section 2. DECLARED 2026-09-15 — the owner updated both consoles and Apple approved 1.16.0 on that basis, so this is now a CONFIRM-IT-IS-STILL-THERE check, not an add-it-now one. It stays a SKIP because nothing here can read either console. The reasoning is kept because it re-arms the moment a round changes what the app collects about a child: the build being submitted is the first one that collects the field, so sending it with either form un-updated hands the reviewer a declaration the binary contradicts. The 2026-09-08 OTA freeze on the 1.15.0 runtime is discharged — the version moved, and runtimeVersion:appVersion means an update published now cannot reach a 1.15.0 binary — but the duty it protected became this one (root CLAUDE.md, \"Releasing a new mobile version\").",
  );
  record(
    SKIP,
    "data-safety declarations cover the Sentry crash reports",
    "The app links @sentry/react-native (wired 2026-09-11). It sends NOTHING unless EXPO_PUBLIC_SENTRY_DSN is set for the build being submitted, so the first question is which way that variable is set on the EAS profile this binary came from — and that is not readable from here. If it IS set, Sentry is a third-party recipient of crash and diagnostic data and BOTH forms are wrong until they say so: Play Data safety adds App info and performance > Crash logs AND Diagnostics; App Store Connect App Privacy adds Diagnostics > Crash Data AND Other Diagnostic Data. Both answer NOT LINKED to the user and NOT used for tracking, which is only true because the SDK never calls setUser, withholds IP from the payload (sendDefaultPii: false) and runs every payload through mobile-app/src/lib/sentryScrub.ts — do not change that config and leave these answers standing. Purpose is App functionality, not Analytics. The Sentry organisation must use the EU (Ireland) data region and Prevent Storing of IP Addresses must remain ON, because the ingest server can infer an address the SDK withheld. The inventory is mobile-app/markdowns/STORE_LAUNCH_PACK.md section 2.6. The trilingual privacy policy already names Sentry and accurately describes its data handling; only the two store declarations remain.",
  );
  record(
    SKIP,
