...
//
// ---------------------------------------------------------------------------
// THE ARITHMETIC — the numbers in sentryOptions.ts are derived, not guessed
// ---------------------------------------------------------------------------
...
// offender it is no longer novel — so what the global cap throttles is exactly
// what it should: the SAME faults, again. And the worst case stays arithmetic
// rather than hopeful:
//
//     max per process per hour  =  perHour + novelPerHour
//     max per process per day   =  perDay  + novelPerDay
//
// With the numbers shipped in sentryOptions.ts that is 24/hour and 70/day on
// the server (20 + 4, 60 + 10) and 13/hour and 38/day in a browser tab
// (10 + 3, 30 + 8). The web app's slice of the org-wide 5,000/month is ~2,400,
// i.e. ~80/day across the whole fleet, so one runaway process still cannot
// outspend roughly a single day of it — which was the property the reserve had
// to preserve while it stopped swallowing novel faults.
//
