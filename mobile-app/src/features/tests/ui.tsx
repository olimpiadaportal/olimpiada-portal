// TEST ENGINE (M3) — arena-styled building blocks for the test screens. All
// colors come from arenaTokens (web .arena parity: dark palette + light remap);
// nothing here hardcodes a literal outside the token maps.
...
export function Eyebrow({ arena, children }: { arena: ArenaTokens; children: string }) {
  return (
    <AppText
      variant="label"
      color={arena.lime}
      style={{ textTransform: "uppercase", letterSpacing: 1.2, fontSize: 12 }}
    >
      {children}
    </AppText>
  );
...
export function Notice({
  ...
  return (
    ...
    >
      <AppText color={arena.ink} style={{ fontSize: 14 }}>
        {children}
      </AppText>
    </View>
  );
...
/** Web .tst-pill — status chip on history rows / review cards. */
...
/** Arena-toned button (web .arena-btn / -ghost / danger cancel). */
export function ArenaButton({
  ...
  const bg = kind === "primary" ? arena.blue : kind === "danger" ? tint(arena.red, 0.14) : "transparent";
  const border = kind === "primary" ? arena.blue : kind === "danger" ? tint(arena.red, 0.5) : arena.line;
  const fg = kind === "primary" ? "#ffffff" : kind === "danger" ? arena.red : arena.ink;
...
export function BackBar({
  ...
  return (
    ...
    >
      <AppText color={arena.lime} style={{ fontSize: 20, lineHeight: 22 }}>
        ‹
      </AppText>
...
/** Progress bar (web .arena-bar) for the per-topic result breakdown. */
