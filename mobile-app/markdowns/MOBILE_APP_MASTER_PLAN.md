...
### 7.2c Round-20 platform changes → the M3.1 PARITY PASS (2026-07-12 — required before M4 ships)
Round 20 reshaped the test/leaderboard model AFTER mobile M3 shipped. The M3 screens keep working (RPC changes were signature-compatible; new payload keys are additive) but are now BEHIND the web. M3.1 must bring parity:
1. **Daily rated rounds are THE rated play**: new RPC `start_daily_round_attempt(p_subject_id, p_day 'today'|'yesterday')` → same runner contract (+`rated` flag; today = timed 25min, ONE per subject/day — errcode unique_violation when consumed; yesterday = untimed unlimited practice replay of the stored snapshot, never rated; no_data_found when no round existed). Tests tab restructures to Today's Rounds / Previous Day's Rounds (with the "practice only — never affects the leaderboard" notice) / Recent, plus a per-subject Practice entry into the old setup flow.
2. **Topic tests are UNTIMED practice now**: `start_topic_test_attempt` returns `deadline_at:null, duration_seconds:null, rated:false` — the M3 runner MUST handle a null deadline (no countdown chip → "no time limit" pill, no auto-submit, no resync); rated/practice badges per attempt.
3. **Points/streak come ONLY from rated attempts** (daily rounds + olympiads) — arena ministats/copy implying topic tests score must change.
4. **Olympiad attempts contain ALL package questions** (count varies — never assume 25; the start payload's `count` is authoritative).
5. **5 answer options (A–E)** everywhere (was 4); option lists are data-driven so rendering just works — but any layout assuming 4 rows must be checked.
6. **Question images**: `get_test_attempt`/`get_test_review` questions now carry optional `image {bucket, path}` — render between body and options (tap = full-screen viewer), runner + review.
7. **Leaderboard**: `get_leaderboard` rows gained a `district` column (after city; derived from the school) + new scope `'district'` (ids from the public-readable `city_districts` table); NO medals — numeric ranks only; top-50 with internal scroll + sticky header; my-rank card unchanged (`{rank,total,value}` — render "#r / total").
8. **Terms (Rüb)**: content-side only (admin manages; generation is server-side) — mobile needs no term UI, but never filter/derive pools client-side.
9. **Maintenance latency**: web moved to a ~4s TTL + polling; mobile's `get_mobile_config` maintenance gate should adopt a similar short refetch cadence on foreground (M3.1 or M4).
10. **Public landing top-10** (anonymized `get_public_leaderboard`, anon-callable) — optional on mobile welcome screen (owner call at M3.1).

### 7.2d Round-21 platform changes (2026-07-13) — folded into the SAME M3.1 pass
11. **Real olympiad question counts**: NEVER display `olympiad_packages.questions_per_attempt` (display-legacy, default 25, never written by admin). Mobile has the bug at `src/features/olympiads/data.ts:19,37,48` (`?? 25`) and `OlympiadsScreen.tsx:312,422-423`. Use the new RPC `get_olympiad_pool_counts(p_package_ids uuid[])` (authenticated) → `{package_id, question_count}` of PUBLISHED pool questions; a package missing from the result = 0.
12. **Tests tab pre-flight**: new RPC `get_my_round_readiness()` (authenticated) → per active subject `{subject_id, round_exists, attempted, ready}` for the calling student's grade. Render a disabled "round not ready yet" state instead of letting Start hit the `no_data_found` error (web Round-21 pattern). Keep the error mapping as fallback.
13. **Arena home layout parity**: web REMOVED "Today's Rounds" and "Latest News" from the student dashboard (news stays on its own tab). Kept + rebalanced: hero/welcome + country-rank card (now wired to the REAL all-time global rank via `get_my_leaderboard_rank` period `all_time`), stats ticker, monthly-ranking + subject-strength side by side, Recent Rounds full-width. The shipped M3 arena home ports the OLD layout — mirror the new one.
14. **Add-child wizard gains a mandatory District (rayon) step** (M2 screen): between City and School, ONLY when the chosen city has active `city_districts` rows (public-readable table; naming trap: `districts` = cities, `city_districts` = rayons). Post `city_district_id`; the BFF `/children` endpoint must pass it to `createChild` — the underlying `create_child_account` is now 11-arg and REJECTS a missing rayon for such cities (check_violation, hint `district_required`). School list filters by the chosen rayon but keeps `city_district_id IS NULL` schools selectable (pending admin review). Edit-child preselects the saved rayon (`students.city_district_id`).
...
| Ranking | `leaderboard` off → GateNotice; Points/Streak boards; grade/city/school scope chips only for ids the child has; Subject = single-select over ALL active subjects (clamped default); "Firstname L." rows + context columns |
