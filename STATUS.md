...
- Stage: Stage 7 — Business-Model Database Foundation — IN PROGRESS.
- Increment 1 (child accounts) DONE on dev/staging: migration `2026_06_27_006_child_accounts.sql` — students child fields (8-digit `child_unique_id`, `created_by_parent_profile_id`, first/last name, city, school, class, `access_status`), `child_unique_ids` registry + `allocate_child_unique_id()` random 8-digit generator (smoke test PASS), `child_credentials` mapping, `wallpapers` catalog + `child_wallpaper_selections`, `wallpaper-assets` storage bucket, RLS (parent manages own children; child manages own wallpaper; credentials/IDs admin/service-role only), 6 solid-color wallpapers seeded. Backport into canonical `002`/`003`/`009`/`010`/`011`/`012` PENDING (at Stage 7 close).
- Remaining Stage 7 increments: (2) child subscriptions + subject pricing + trial/promo + payments + checkout + sibling-discount (extend `007`); (3) News (`014`); (4) Olympiad Preparation (`015`). Then backport all + validate.
