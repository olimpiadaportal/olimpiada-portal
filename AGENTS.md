# AGENTS.md — OlympIQ Codex Instructions

## Codex entrypoint

This is the root entrypoint for Codex working in `olimpiada-portal/`. The root
`CLAUDE.md` is reproduced verbatim below so its project rules are preserved.
References to Claude Code as the coding tool apply to Codex as well. The old
sentence naming Claude Code as the only planned agent is superseded by the
owner's explicit Codex handoff; it does not prohibit work in Codex.

Read the complete root `CLAUDE.md` before implementation, even if automatic
instruction loading truncates this long file. Read the current resume section
of `STATUS.md`, inspect the actual Git working tree, and then read the required
stage documents. Historical status text is evidence to verify, not proof of
what is currently committed, deployed, or applied to a database.

Before working within a directory, also read its existing scoped instructions:
...
Read each applicable file when a task crosses those boundaries. These files
remain authoritative project guidance; they have not been replaced or edited.
Follow the current user's scope. A review-only request does not authorize
implementation, migrations, releases, or unrelated documentation updates.
The implementation-time STATUS/CHANGELOG requirements below apply when the
owner authorizes implementation.

## Resolve historical contradictions before acting

Prefer the owner's latest explicit decisions and the newer dated project rules
over older baseline planning text. The copied source contains historical
statements such as mobile being dormant; the actual mobile app is implemented.
The mobile-specific charcoal dark theme is an explicit owner override, while
the web dark palette remains unchanged. Do not revert current behavior merely
to match an older planning paragraph.

Database names are literal: `OLIMPIADA_PROD_DB_URL` is PRODUCTION and
`OLIMPIADA_STAGING_DB_URL` is STAGING. `docs/DEVELOPER_SETUP.md` still describes
the production variable as dev/staging; that description is obsolete. Follow
the newer database rules below. Check credential presence without displaying
values. Never infer connectivity or migration state from a variable being set.

## Current continuation context reviewed on 2026-09-13

Recheck Git, `STATUS.md`, and the relevant source before continuing; this
section records the owner decisions that completed the Codex handoff rather
than replacing those live sources.

The Subjects/pricing merge, first-launch language selection, sibling prefill,
and shared-child access are implemented for unreleased mobile 1.16.0. A second
parent or guardian links the same child through a one-time code and owner
approval, sees the child's progress and existing access without another
payment, and cannot manage creator-owned finances. The lifecycle includes
approval, revocation, self-unlinking, safe creator deletion, surviving-parent
promotion, and audited admin support. Migrations 173–176 were repaired,
rehearsed, backported, and applied to staging and production on 2026-09-12;
production checks 130–134 passed. They are no longer held.

The iOS subscription surface follows the admin free-access state, pushed mobile
screens have explicit back navigation, and portrait is enforced through Expo
config, runtime locking, iPad full-screen mode, and Android 16's temporary
large-screen compatibility property. The full validation record and remaining
physical-device acceptance work are in `STATUS.md`.

Gender remains OPTIONAL. Google SSO and store-console work are deferred by the
owner. No teacher role or teacher-sharing flow was added. Do not silently
reinstate mandatory gender, SSO, the discarded password-based child-link design,
or the older 1.17.0 plan.

## Root project instructions — verbatim CLAUDE.md
...
- **TWO DATABASES, AND THE NAMES ARE NOT INTUITIVE (owner decision, 2026-08-04) — read this before any `psql`:**
  - **`OLIMPIADA_PROD_DB_URL` is PRODUCTION.** It is the live `eu-west-1` (West EU, Ireland) Supabase project that serves `olympiq.ai`, the mobile app and the admin panel. It holds real accounts. *(Historical note: until 2026-08-21 this connection string was still exposed under a name ending in `_DEV_DB_URL`. That variable has now been DELETED from the environment and `OLIMPIADA_PROD_DB_URL` is the only name for production. If you ever find a `_DEV_DB_URL` again, treat it as PRODUCTION until proven otherwise — reading "DEV" and assuming a sandbox is the mistake this rename exists to prevent.)*
  - **`OLIMPIADA_STAGING_DB_URL` is the SANDBOX** — the Supabase project named **OlympIQ Staging** (created 2026-08-21, `eu-west-1`, free tier, schema-only, no real data). Anything destructive goes here. Before the first destructive statement of a session, CONFIRM the two URLs resolve to different projects; the cost is one query and the failure it prevents is unrecoverable.
...
- **MIGRATIONS APPLY BEFORE THE CODE THAT NEEDS THEM DEPLOYS.** Vercel auto-deploys on push, so
  ...
  still unapplied: every AzeriCard payment endpoint returned "unknown order" until the migration
  ran. Database first, then push — never the reverse. A database ahead of its code is inert; code
  ahead of its database is broken.
...
- **GENDER DECLARATION BLOCKER (2026-09-09). The OTA freeze that stood here is DISCHARGED; the obligation behind it is not — it has become a pre-submission blocker for 1.16.0.** `expo.version` left 1.15.0, and `runtimeVersion: appVersion` means an update published now carries runtime 1.16.0 and can never reach a 1.15.0 binary, so the over-the-air hazard the freeze existed for is genuinely spent. What it was protecting is not: **1.16.0 CONTAINS the optional child `gender` field — a MINOR’s personal data — while Play *Data safety* and App Store *App Privacy* still answer that no such data is collected.** Shipping it over the air onto a live 1.15.0 build would have made those answers false; SUBMITTING a new build that collects it makes them false on a form both stores read as part of the review, which is worse, not better. **So both declarations are updated BEFORE 1.16.0 is submitted to either store** — before, not alongside, and never after. Inventory to fill them from: `mobile-app/markdowns/STORE_LAUNCH_PACK.md` §2 · console-by-console steps: `docs/STORE_LISTING_COPY.md` §8.1 · `node mobile-app/scripts/submission-preflight.mjs` names it among the checks no script can verify, and a SKIP there is not a pass. The OTA reasoning returns, and this bullet has to become a freeze again, if either `expo.version` goes back to 1.15.0 or `runtimeVersion` moves off the `appVersion` policy — both make 1.15.0 binaries reachable once more. Delete this bullet only when BOTH forms list gender, and delete `mobile-app/__tests__/ota-data-safety-freeze.test.ts` in the same change: it fails if the blocker goes missing, or stops naming the version it blocks, while the app still collects the field.
...
- **Architecture of record, SPLIT BY PLATFORM since 2026-09-04 — read the split before touching any commerce code:**
  - **iOS SELLS.** The binary ships StoreKit in-app purchase: 21 non-renewing subscriptions (7 subjects × week/month/year), bound to one child through `appAccountToken`, verified server-side against Apple before access is granted. This answers the **Guideline 3.1.1 rejection of 2026-08-31**, and Apple **approved 1.15.0 and all 21 products on 2026-09-09**. Do NOT strip it, do NOT reintroduce purchase-silence on iOS, and do NOT read the Android rule below as applying to it. The old blanket "the mobile apps are purchase-silent" wording lived here until 2026-09-08 and is exactly the sentence that would tell a future session to delete working, approved code.
  - **ANDROID STAYS PURCHASE-SILENT.** No price, no buy CTA, no `olympiq.ai` in a purchasing context — there is no IAP on that binary, and Google's anti-steering rules apply in full in Azerbaijan. Parent tabs included: Google's consumption-only test is app-wide, and parent and child share one binary.
  - **THE WEB RAIL IS UNCHANGED** (browser, ABB, AZN) and remains the only rail for olympiad packages.
  - The platform split is a BUILD-TIME fact, not a runtime flag — see the next bullet, which still holds.
- **The payment RAIL is a BUILD-TIME constant.** Runtime config may close/hide the approved iOS StoreKit rail during giveaway, scheduled free-access or payment-off states, but it must never reveal a web/non-IAP checkout or make Android sell. A non-IAP purchase flow sitting in a store binary behind a remote switch is Apple 2.3.1(a) — the penalty is developer-account termination, not rejection.
- **Never in a store build:** an AZN price anywhere in the app; a Subscribe/Abunə ol/Yenilə/Əldə et button; the `olympiq.ai` URL or a QR code in a purchasing context; a webview checkout or card form; fake prices/invoices/simulated cards; an external `https` link opened from notification content or any admin-controlled string; telling a child to ask a parent to **buy** (use access/activation language). **The iOS IAP rail does not breach any of these**, and the reason is worth keeping: every price it shows is StoreKit's own localized string — the app displays no price of its own, holds no card form, and opens no external URL. A CHILD still has no purchase surface on either platform.
...
- **Entitlement is its own provider-agnostic table** (`source ∈ abb_web | apple_iap | google_play | giveaway | manual | school_license`). An ABB subscription row must NEVER *be* the entitlement — that is what makes a forced-IAP scenario a two-week job instead of a rewrite, and it is precisely what happened: the 2026-08-31 rejection was answered in days because `apple_iap` was already a value this table understood. The old "do not pre-build a dormant IAP path" clause is RETIRED — the path is built, live and approved. What replaces it: every rail writes an entitlement and NOTHING reads a rail directly, so a third provider is a new `source` value and a grant function, not a migration of the access model.
...
- Never delete purchased olympiad package records; archive listings only (purchasers keep lifetime access). This rule held through the 2026-07-30 curriculum purge and was NOT relaxed: migration `094` emptied the 8 packages' question POOLS but deleted no `olympiad_packages` and no `olympiad_purchases` row, and left the packages ACTIVE on purpose so purchasers keep seeing what they bought (`can_view_olympiad_package()` grants access through the purchase branch, which never reads `status`). Those 8 packages now need a replacement pool of at least `questions_per_attempt` per target grade before they can serve an attempt. Olympiad BULK upload APPENDS into an existing target grade's pool (owner decision 2026-08-11, migration `108`; supersedes the migration-059 creation-only rule) — a row whose content (same primary-locale body, same option texts, same image references) was ALREADY IN that (package, grade) pool when the call started is reported as a row error and skipped, so re-uploading a file is safe. The comparison is against the PRE-EXISTING pool ONLY: rows inserted by the same call are never compared with each other, so a file containing the same question twice imports both — package creation and add-grade are all-or-nothing, and flagging an in-file duplicate would roll back a creation that previously succeeded. Rows carrying images cannot be compared (media uuids are minted per upload) and are never flagged; the key is bound to the primary locale on both sides (the same question re-uploaded with a different `primary_locale` does not match); the guard is best-effort, not a DB constraint. Admins also manage individual pool questions via the package's question CRUD (Round 21) — edits keep option ids stable (never delete+reinsert options) and answered questions can only be archived, never deleted. Displayed question counts: the REAL published pool count (`get_olympiad_pool_counts`) stays the headline number; since Round 51 `questions_per_attempt` is LIVE again and may be shown ONLY as its own clearly-labelled per-attempt row ("Hər girişdə sual sayı"), never in place of the pool count.
