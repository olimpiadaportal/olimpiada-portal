...
-- -----------------------------------------------------------------------------
-- admin_replace_olympiad_grade_pool : replace ONE grade pool, atomically.
--
-- Backported from migration 2026_08_27_147. The full reasoning lives in that
-- file; the load-bearing parts, so nobody has to go and find it:
--   * ONE transaction. delete-then-upload from the client leaves a window in
--     which the pool is EMPTY and every lifetime purchaser who opens the
--     olympiad gets no_data_found.
--   * STRICTLY all-or-nothing, unlike the append importer it calls.
--   * Answered questions are ARCHIVED, never deleted -- an olympiad attempt has
--     no content snapshot, so deleting one destroys the graded result.
--   * Archived survivors are hidden from the importer's duplicate snapshot for
--     the duration of the import, then re-attached (asserted). Without that a
--     re-uploaded question is skipped and the replacement fails its own count
--     check.
--   * Asserts the per-attempt floor against the POST state, which NOTHING else
--     checks on a shrink: start_olympiad_attempt clamps instead of refusing.
-- -----------------------------------------------------------------------------
