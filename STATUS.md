...
- NEXT implementation is the **revised Stage 7 — Business-Model Database Foundation** (NOT the old Stage 7 test engine): migrations + new canonical SQL (`014_news.sql`, `015_olympiad_preparation.sql`, extend `002`/`007`/`009`/`010`/`011`) for parent/child accounts, child subscriptions/payments, news, olympiad preparation, wallpapers. Begin only on approval (Prompt 2).
