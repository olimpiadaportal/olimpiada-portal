// World calling codes for the parent phone field (Round 11, owner item 3).
// Static ITU E.164 country-code assignments — { iso2, name (English), dial }.
// Shared codes get one entry PER country/territory (e.g. +1 NANP members, +7
// RU/KZ, +44 GB/GG/JE/IM, +61 AU/CX/CC…). Localized display names come from
// Intl.DisplayNames at render time (see PhoneField); `name` is the fallback.
// Notes: Vatican City is listed under +39 (its numbers live in the Italian
// plan); Kosovo (XK, +383) uses the user-assigned ISO code.

export type Country = {
  /** ISO 3166-1 alpha-2 code (uppercase). XK = Kosovo (user-assigned). */
  iso2: string;
  /** English short name (fallback when Intl.DisplayNames is unavailable). */
  name: string;
  /** International dial code, digits only (no "+"). */
  dial: string;
};

export const DEFAULT_ISO2 = "AZ";
...
/** All countries, alphabetical by English name (UI may re-sort by locale). */
