...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!subscription?.stripe_subscription_id) {
      return json(req, { error: 'Active subscription not found' }, 404);
    }

    if (subscription.cancel_at_period_end) {
      return json(req, { subscription, alreadyScheduled: true });
    }
    ...
    const response = await fetch(
      `https://api.stripe.com/v1/subscriptions/${subscription.stripe_subscription_id}`,
      {
        ...
        headers: {
          ...
          'Idempotency-Key': `cancel_teacher_subscription_${subscription.id}_${new Date(subscription.updated_at).getTime()}`,
        },
    ...
    if (!response.ok) {
      return json(req, { error: stripeSubscription?.error?.message || 'Cancellation failed' }, 502);
    }
    ...
    return json(req, { subscription: saved, alreadyScheduled: false });
  } catch (error) {
    console.error('cancel-teacher-subscription error:', error);
    return json(req, { error: error instanceof Error ? error.message : 'Cancellation failed' }, 500);
  }
});
