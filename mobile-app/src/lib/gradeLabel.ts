...
/**
 * Round 34 (web parity): label for a MULTI-grade set. Contiguous levels render
 * as a range ("4–6-cı siniflər"), sparse sets as a list; a single level falls
 * back to formatGradeLabel.
 */
export function formatGradeRangeLabel(levels: number[], locale: Locale): string {
  const sorted = Array.from(
    new Set(levels.filter((n) => Number.isInteger(n) && n > 0)),
  ).sort((a, b) => a - b);
