...
- **Public** (unauthenticated): landing-lite, pricing (per-subject plan cards), about highlights, FAQ, contact (admin-driven email/phone/socials), public news (views + like counters), login (parent|student segmented), parent registration (phone OPTIONAL since 2026-08-31 — Apple 5.1.1(v)).
...
- **Light mirrored, dark mobile-authored** — `src/theme/tokens.ts` replicates the LIGHT half of `web-app/src/app/globals.css`; the dark half deliberately diverges (see the dark bullet):
  ...
  - **Dark (NEUTRAL CHARCOAL — owner override, 2026-09-10; do NOT re-sync it to the web):** the web dark palette is blue-black and the owner reported it reading as dark-BLUE, so mobile's dark surfaces and neutral inks were replaced with luminance-matched near-greys. App: `#100e0e/#1f1d1d/#262625/#323230/#f3f3f1/#9b9999/#e2e0e0`. Arena: `#100e0e/#191717/#1f1d1d/#262625/#323230/#f3f3f1/#8f8d8d/#646463/#c4ff00/#2f6bff/#ff4d6d/#ffc94d`. Brand accents keep their hue, the blue ones included (accent `#2f6bff`, arena lime/blue/red/gold, and the accent ink `pillText`) — "neutral" describes the surfaces, not the brand. **CHROMA IS BUDGETED BY ROLE and there are THREE tiers, not two** (corrected 2026-09-10 after the first pass shipped with two): neutral surfaces and neutral inks at C*ab <= 1.4, brand accents at C*ab >= 30, and exactly one TINTED SURFACE between them — `pillBg` `#232532` at C*ab 9.2, the accent hue at surface strength. Filing that pill as an accent is what exempted it from the sweep and left it at the web's C*ab 24.7, reading as a foreign navy block against neutral neighbours; the re-tint held L*ab at 15.01, so `pillText` on `pillBg` moved 8.2899:1 -> 8.2900:1. Every swap is luminance-matched, so no contrast ratio moved by more than 0.03 and no WCAG grade changed. **The WEB dark tokens and the web `.arena` palette remain the owner's frozen reference and are untouched** — root `CLAUDE.md`'s freeze governs `web-app/`, and the change cannot leak there (`web-app/scripts/gen-palettes.mjs` emits light only; nothing in `mobile-app/` reads `globals.css`). Surface-vs-surface separation must also survive without hue: the student home rank ring's track moved `arena.bg2` -> `arena.bg` (1.064:1 -> 1.147:1) because a bare arc on a panel has no border or shadow to carry the split, and it goes DOWN not up because the brand sweep painted on it measures 3.38:1 over `bg` but 2.26:1 over the lighter `line`. Pinned by `__tests__/dark-theme-neutrality.test.ts`. Known open item: `ARENA_DARK.dim` is 2.83:1 on `panel` — a pre-existing hierarchy/AA trade-off the hue fix did not change, awaiting an owner decision. arena-LIGHT remap stays identical to the web `[data-theme="light"] .arena` block (lime→purple, white panels).
...
- **`get_mobile_config()`** — SECURITY DEFINER, **anon-callable, hard-coded whitelist** (never `select *`), returns one JSON:
  ...
  The payment **mode** is resolved server-side exactly like `web-app/src/lib/paymentMode.ts` (giveaway window = `giveaway.started_at` + `giveaway.duration_days`, lazy expiry) — the client NEVER computes or trusts a mode. Fetched at cold start + foreground (React Query, 5-min stale).
  > **SUPERSEDED 2026-08-18 (owner): the `demo` mode is DELETED platform-wide.** The modes are **`real | giveaway | off`** (precedence giveaway>real>off); `demo_payments` no longer exists as a flag, `PaymentMode` no longer has the member, and a server that still said `demo` degrades to `off`. Every "demo"/"mode-driven purchase flow" sentence below is historical.
...
| Register | OPTIONAL phone (PhoneField; Apple 5.1.1(v)), email-exists message (owner-approved exception), rate-limit errors |
...
| Add-Child wizard | *(2026-08-18: ALWAYS info→done — the demo purchase wizard is deleted)* giveaway/free-access = info→done (instant ID); real/off = info→done (login ID pending); ID-reveal screen (copy-to-clipboard + sensitive warning) |
...
| Olympiads catalog | *(2026-08-18: BROWSE-ONLY in every mode — no price chip, no buy CTA, no purchase call)* `olympiad_module` off → GateNotice; unlocked pill; store-compliance posture per `docs/STORE_PAYMENTS_COMPLIANCE.md` |
| Subscription | *(2026-08-18: the fabricated Billing/Invoices sections and the three-way switcher are DELETED, and the card shows status + coverage end, never an amount)* child-selector chips; live sub → manage-subjects (price-free) + cancel; `real` → read-only notice |
...
> **SUPERSEDED (2026-07-26) by `docs/STORE_PAYMENTS_COMPLIANCE.md`** — read that document, not this section, before touching anything commerce-related. It is the authoritative, sourced analysis of Apple/Google payment policy for the **Azerbaijan** storefront plus the ABB web rail. The guidance previously in this section was researched before the 2024–2026 policy changes and is wrong in two material ways, recorded here so the error is not repeated:
>
> 1. It said `demo`/`giveaway` flows **MAY** run end-to-end in a store build "since no real money moves". They may not. A non-IAP purchase flow present in a store binary and switched by a **server flag** is Apple Guideline 2.3.1(a) (hidden/dormant features) — the downside is developer-account termination, not a rejection. Payment posture must be a **build-time constant** with demo/subscribe code dead-stripped from store builds.
> 2. It recommended hiding purchase buttons behind *"managed from the family's web account"* wording. That names a destination, and rendered next to a price it is a call to action. On the Azerbaijan storefront there is **no** anti-steering relief (Apple 3.1.1(a) applies in full; the *Epic v. Apple* carve-out is US-storefront-only and the DMA is EEA-only). Correct copy names no destination: *"Abunəliklər bu tətbiqdə idarə olunmur."*
>
> Also do **not** reach for Guideline 3.1.3(b) Multiplatform Services as the justification for shipping without IAP — its verbatim proviso requires the items to *"also [be] available as in-app purchases within the app."* Citing it argues *for* IAP.

**Architecture of record:** all purchasing happens on the **web** in a browser, paid via **ABB**; the mobile apps are **purchase-silent** and reflect entitlement only (Google explicitly permits consumption-only apps; Apple tolerates the 3.1.3(f) shape with real, recurring rejection risk). Because parent and child share one binary, the **parent tabs must be purchase-free too** — Google's consumption-only test is app-wide, not role-wide. Conversion happens *outside* the app (email/web/schools), which both stores permit without restriction.

**Current reality:** NO real payment provider exists anywhere in the platform (backlog A1). The mobile binary today still ships a working non-IAP checkout behind the runtime `PaymentMode` flag — see the verified gap analysis in `docs/STORE_PAYMENTS_COMPLIANCE.md` §7. That is launch-blocking and must be resolved before any store submission.

**Forward design for real payments:** entitlement becomes its own provider-agnostic table (`source ∈ abb_web | apple_iap | google_play | giveaway | manual | school_license`) so ABB is one *producer* of entitlement rows and never the source of truth for access. That makes "add IAP on iOS only" a new producer rather than a re-architecture. Do **not** pre-build a dormant StoreKit/Play Billing path — same hidden-feature problem in reverse. BFF purchase endpoints keep the Idempotency-Key header from day one. Store listing: az/en/ru metadata, screenshots per theme, age rating questionnaire — and **do not opt into Kids Category / Designed for Families** (the 1.3 commitment is sticky and would permanently foreclose adding IAP without a parental gate).
