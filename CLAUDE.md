...
- Users never choose question difficulty; question sets are always server-side selections. Round-38 model (owner, 2026-07-24; supersedes the Round-20 shared-snapshot clause): RATED play = one SUBMITTED daily round per subject per day (a PER-STUDENT subtopic-balanced random 25 drawn at start from the cumulative-term published pool, timed 25 min, feeds the percentage boards/streak; the day is consumed ONLY by submit — a live attempt resumes, an abandoned/expired one costs nothing and the next start draws a FRESH set) + purchased olympiads (attempts include ALL of the package's questions — no fixed count). Topic tests and previous-day round replays are UNTIMED practice that never affects points/percentage/streak/boards; yesterday's replay = the student's LOCKED practice set (own submitted set → a peer's set → generated; `daily_practice_sets`). Fairness across differing sets comes from the Round-36 weighted-percentage ranking.
