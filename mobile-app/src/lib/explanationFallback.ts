...
// fallback. The impure half (the query) stays in api.ts fetchTestReview.
//
// KEEP IN SYNC with web-app/src/lib/explanationFallback.ts (same rule as
// localizedName/topicStanding): mobile cannot import from web-app, only i18n
// crosses over through sync-i18n. The two must label identically or the same
// attempt reads as translated on one platform and untranslated on the other.
