-- Migration: 2026_09_16_178_child_gender_in_create.sql
-- Purpose: Carry the child's gender INSIDE the provisioning transaction, so a
--          field the product now requires cannot be silently lost.
-- Environment first applied: (pending) staging, then production.
-- Related root SQL: 011 functions; 013 checks 66 and 127.
-- Backport status: pending. Destructive change: no (drops one stale overload).
-- Requires 169 and 177. Idempotent. No BEGIN/COMMIT inside this migration.
-- DOWN: recreate the 11-argument overload from 011 and drop the 12-argument one.
--       No data is lost either way; students.gender stays nullable throughout.
--
-- WHY. Gender was written by a SEPARATE service-role patch AFTER this function
-- had already committed, and a failure there was deliberately NON-FATAL: the
-- child existed, so the miss surfaced as a warning (addchild.warn.genderNotSaved)
-- rather than rolling anything back. That is the right shape for an OPTIONAL
-- field and the wrong one for a required field - "the form made you answer and
-- then discarded the answer" is a data bug the parent never sees. The value now
-- travels in the same transaction as the row it belongs to.
--
-- THE COLUMN STAYS NULLABLE, WHICH IS NOT AN OVERSIGHT. Production holds 51
-- children created before the question existed. Migration 169's header argues
-- that inventing a value for them yields "a number that looks like data and is
-- not", and there is no honest backfill for a minor's personal data. So the
-- requirement lives where it is true - at the form and the server, for NEW
-- children - and 013 check 127 still fails if anyone makes the column NOT NULL.
--
-- THE BODY BELOW IS THE LIVE FUNCTION, taken from pg_get_functiondef on
-- production and modified in exactly four places: the 12th parameter, one
-- declaration, the validation block, and the gender column in the INSERT.
-- Nothing else was retyped, because a hand-rebuilt body is how a guard quietly
-- disappears.
...
begin
  ...
  if p_gender is not null and btrim(p_gender) <> '' then
    if p_gender not in ('female', 'male') then
      raise exception 'create_child_account: gender must be female or male'
...
-- Drop the superseded 11-argument overload. Left in place, PostgREST could still
-- resolve a call to the old body, which writes no gender at all - the silent
-- loss this migration exists to end. 013 check 66 pins both halves.
drop function if exists public.create_child_account(uuid, uuid, text, text, text, text, text, uuid, uuid, uuid, uuid);

-- Same posture as the function it replaces: never reachable by a client.
