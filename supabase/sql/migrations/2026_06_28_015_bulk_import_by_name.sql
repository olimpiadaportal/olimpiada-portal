-- Migration: 2026_06_28_015_bulk_import_by_name.sql
-- Purpose: "Code" removed from the platform — bulk question import now resolves
--          subject / question type / olympiad type by NAME (not code), and drops
--          difficulty entirely. Codes still exist internally (auto-generated) but
--          authors never need them.
-- Environment first applied: development/staging
-- Related root SQL file(s): 011 (bulk_insert_questions function body).
-- Backport status: pending (at this increment's close)
-- Destructive change: no (CREATE OR REPLACE of an existing function)
-- Rollback notes: re-apply the prior (code-based) version of bulk_insert_questions.
-- Item format now: { primary_locale, meta:{ subject, grade_level, type,
--   olympiad_type?, topic?, subtopic?, source? }, translations:{az:{body,prompt?,
--   explanation?},en?,ru?}, options:[{is_correct,order_index?,text:{az,en?,ru?}}] }
-- =============================================================================
...
begin
  ...
  loop
    v_idx := v_idx + 1;
    begin
      -- ---- required taxonomy, resolved BY NAME ----
...
-- =============================================================================
-- End of 2026_06_28_015_bulk_import_by_name.sql
-- =============================================================================
