...
### Stage 8 — Child Authentication & Account Model (ACTIVE, started 2026-06-28)
...
- **Increment 8.2 — Server service layer (next, no UI):** server-only service-role admin client; `createChild` (admin.createUser synthetic `c<8digits>@children.invalid` + parent password → `create_child_account` RPC, with auth-user cleanup on RPC failure), `childLogin` (8-digit ID → synthetic email → `signInWithPassword`, lockout-gated), `resetChildPassword` (service role); zod schemas (password ≥8, ≠ ID; 8-digit format); trilingual auth error strings (az/en/ru).
